...
import { saveSeasonCustomer } from '../../../services/seasonCustomersService';
...
class AddSeasonCustomer extends Component {
	constructor(props) {
		super(props);
	}
	state = {
		...
		return (
			<Fragment>
				<Row>
					<Colxx xxs='12'>
						<Breadcrumb
							heading='menu.add-season-customer'
							...
							<CardBody>
								...
								<Formik
									...
									onSubmit={async (values) => {
										...
										try {
											...
											if (response) {
												...
												this.props.history.push('season-customers');
											} else {
									...
									render={({
										...
										let carparks2 = this.state.Carparks.filter(
											(c) => c.CompanyId?._id === this.state.selectedOption
										).map((c) => {
											return { value: c._id, label: c.CarParkName };
										});
...
export default connect(mapStateToProps)(AddSeasonCustomer);
