...
class Sidebar extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      viewingParentMenu: '',
      collapsedMenus: [],
      menuItems: menuItems.filter(item => item.roles && item.roles.includes(props.role)).map(item => {
        item.subs = item.subs && item.subs.filter(sub => sub.roles.includes(props.role));
        return item;
      })
    };
  }
